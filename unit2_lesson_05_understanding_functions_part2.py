...
def test_function_call_with_keyword_arguments():
    ...
    assert [15,20,30,4] == demo(15,second=20, third=30)
    assert [10,15,30,4] == demo(10,15,30)
    assert [10,15,30,4] == demo(10,15, third=30)
...
def simple_format(format, *args):
    ...
    pass
    for c in format:
       if c.isdigit() and int(c) < len(args):
               input = list(format)
               input[input.index(c)-1] = ''
               input[input.index(c)] = args[int(c)]
               format="".join(input)
    return format
