...
def test_string_formatting_literals():
    ...
    """
    value = 10
    assert 'value is: 10' == "value is: {value}".format(value=value)
    value = 20
    assert "value is: 30" == "value is: {value}".format(value=value+10)
    name, age = "Ramu", 20
    assert "Ramu is 20 years old!"== "{name} is {age} years old!".format(name=name,age=age)
